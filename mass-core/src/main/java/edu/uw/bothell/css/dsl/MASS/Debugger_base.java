...
public class Debugger_base extends Place {
	...
	if(pid == 0){
	    ...
	    System.out.println("places size " + places.places_size);
	    System.out.println("places array size " + places.places.length);
	    System.out.println("place name " + places.places[0].getClass().getName());
	...
	places.places[i].setDebugData(new Double(place_val));
	return null;
    }
	...
	public void run() {
		...
		output.writeObject(places.places[0].getClass().getName());
		...
		synchronized(debuggerInstance){
			...
			synchronized(sending_lock){
				...
				for(int i=0; i<MASS_base.currentReturns.length; i++){
				    Double[] d = (Double[])(MASS_base.currentReturns[i]);
			...
			synchronized(sending_lock){
				...
				int nAgents = 0;
				for(int i=0; i<MASS_base.currentReturns.length; i++){
				    nAgents += ((AgentDebugData[])(MASS_base.currentReturns[i])).length;
				}
				//send the number of agents
				output.writeObject(Integer.toString(nAgents));
				for(int i=0; i<MASS_base.currentReturns.length; i++){
                                    AgentDebugData[] next_agents= (AgentDebugData[])(MASS_base.currentReturns[i]);
