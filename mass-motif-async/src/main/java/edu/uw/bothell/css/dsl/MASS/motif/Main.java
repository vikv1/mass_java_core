...
public class Main {
    ...
    public void run() {
        ...
        GraphCrawler.Constructor constructor = new GraphCrawler.Constructor(motifSize);
        ...
        int[] funcIds = new int[constructor.getSubgraph().order()];
        for(int i = 0; i < funcIds.length; i++) {
          funcIds[i] = GraphCrawler.update_;
        }
        System.out.println(
            (System.currentTimeMillis() - start) + " milliseconds to " +
       ...
       /* int remainingSubgraphs = crawlers.nAgents();
        ...
        try {
          crawlers.callAllAsync(funcIds, null);
        } catch (Exception e) {
          e.printStackTrace();
        }
