...
public class Wave2DMass_Parallel extends Place {
	// constants
	 ...
	 */
	public Wave2DMass_Parallel( Object interval ) {
		//super( interval );
  ...
  public Object doSpecial(Object args){
	...
	public Double exchangeWave( Object args ) {
		return new Double( ( ( time == 0 ) ? wave[0] : wave[1] ) ); //wave one used after start
	}
	...
	public static void main( String[] args ) throws Exception 
	{
		...
		if ( args.length < 9 ) {
			System.err.println( "usage: " +
					...
					"#processes #threads filename showGfx" );
			System.exit( -1 );
		}
		...
		massArgs[3] = port;
		massArgs[4] = "netcdfAll-4.2.jar;hadoop-core-1.1.2.jar;commons-configuration-1.6.jar;commons-lang-2.4.jar";   // optional jar to be loaded with MASS, must be in the same working directory
		MASS.init( massArgs, nProcesses, nThreads );

		// create a Wave2D array
		Places wave2D = new Places( 1, "Wave2DMass_Parallel", ( Object )( new Integer( interval ) ), size, size );
		...
		Places writer = new Places( 2, "Parallel_NetCDF", ( Object )null, size, size );
...
//				}
		...
		writer.callAll(Parallel_NetCDF.flush_, (Object)null); // Write to disk and 
