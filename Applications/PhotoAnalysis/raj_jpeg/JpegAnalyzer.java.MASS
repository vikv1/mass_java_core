...
//  String[] fileName = new String[args.length]; //number of images to\compare
		...
		MASS.init(massArgs, args.length, 1);

      		Places p = new Places(1, "JpegAnalyzer",(Object)args[0], args.length);
		Object[] retval = new Object[args.length];
		for(int i = 0; i < retval.length; i++){
		    retval[i] = new Object();
		}

		p.callAll(0,(Object[])args);

		//for(int i = 0; i < args.length; i++){
		//   p = new Places(1, "JpegAnalyzer",(Object)args[i], args.length);
		//   p.callAll(0,retval[i]);
		//}

		//		System.out.println("retval.length" + retval[0] == null);
		//pics.add( ((JpegAnalyzer)retval[0]).pixelData.getpixels());
		 //System.out.println("pics size= "+ pics.size());

	
		 //for(int i = 0; i < args.length; i++){     
		 // analyzer.add(new JpegAnalyzer(args[i], true, "log.out")); //create an analyzer
		     //  pics.add( ((JpegAnalyzer)retval[i]).pixelData.getpixels()); //get p\ixels of all pics from args
		 //}
	...
	private static final int byte2 = 0xFF00;
	    ...
	    //new JpegAnalyzer((String)fileName, true, "log.out");
	    
	    logging = true;
	    if(logging){
		try{
		    logFile = new PrintWriter(new FileOutputStream("log.out"), true);
		} catch(Exception e){
	    ...
	    try{
		iStream = new FileInputStream((String)fileName);
	...
	JpegAnalyzer(String fileName, boolean logSwitch, String logFileName){		
    	...
    	public Object init(Object fileName){
	    return (Object)(new JpegAnalyzer((String)fileName, true, "log.out"));
       }

    public Object callMethod(int funcId, Object fileName){
    	switch(funcId){
