...
public class Parallel_NetCDF extends Place {
	...
	private Object init(Object arg) {
		...
		if (initialized) return new Integer(-1);
	...
	private Object open( Object arg ) {
		...
		int bufferLoc = index[1] * size[0] + index[0];
		...
		try {
			...
			if (create()) {	// Try to create the file
				try {
					toOpen = NetcdfFile.open(filename);
				...
				if (i < allVar.size()) {	// Only access list if index 'i' is valid
					...
					try {
						if (preconvert) {
							...
							buffer[i] = currVar.read();	// If variable can be read, save Array to buffer
						}
	...
	private Object read( Object arg ) { // read data from a NetCDF file
		if (!opened) return null;	// Return if file has not yet been opened

		Object[] args = (Object[]) arg;
		int[] arrayIndices = (int[]) args[0];	
		int bufferLoc = index[1] * size[0] + index[0];
		...
		synchronized( buffer ) {
			if ( buffer == null ) { // NetCDF file hasn't been read yet
				// Should never happen since open should create the buffer.
				// If read is called and opened is true but buffer is null a serious error has occurred
				return null;
			}
	...
	private Object write( Object arg ) { 
		...
		Object toStore = args[1];

		int rank = indices.length;
		int bufferLoc = (indices[1] * size[0]) + indices[0];
		...
		synchronized( buffer ) {
			...
			Object arrayWNewValue = setNewValue(indices, rank, type, toStore, buffer[bufferLoc]);
			
			if (arrayWNewValue != null) {
				buffer[bufferLoc] = arrayWNewValue;
				edited = true;
				return new Integer(0);	// Returns 0 for success
			}
		}

		return new Integer(-1);	// Returns -1 if could not save new value
	}
	...
	private Object flush( ) { 
		if (!opened) return null;	// Return if file has not yet been opened
		// If nothing has been written to the buffer since it was opened/saved
		// simply returns 0 since there is no reason to open the file
		if (!edited) return new Integer(0);
	...
	private Object setNewValue(int[] indices, int rank, int type, Object newVal, Object bufferArray) {
		switch (rank) {
		case 1: 
			switch (type) {
			case type_int:
				...
				if (preconvert) {
					int[] curr = (int[]) bufferArray;
					curr[indices[0]] = toWriteI;
					return curr;
					
				} else {
					ArrayInt.D1 dataI = (ArrayInt.D1) bufferArray;
			...
			case type_long:
				...
				if (preconvert) {
					long[] curr = (long[]) bufferArray;
					curr[indices[0]] = toWriteL;
					return curr;
					
				} else {
					ArrayLong.D1 dataL = (ArrayLong.D1) bufferArray;
			...
			case type_bool:
				...
				if (preconvert) {
					boolean[] curr = (boolean[]) bufferArray;
					curr[indices[0]] = toWriteB;
					return curr;
					
				} else {
					ArrayBoolean.D1 dataB = (ArrayBoolean.D1) bufferArray;
			...
			case type_double:
				...
				if (preconvert) {
					double[] curr = (double[]) bufferArray;
					curr[indices[0]] = toWriteD;
					return curr;
					
				} else {
					ArrayDouble.D1 dataD = (ArrayDouble.D1) bufferArray;
			...
			case type_float:
				...
				if (preconvert) {
					float[] curr = (float[]) bufferArray;
					curr[indices[0]] = toWriteF;
					return curr;
					
				} else {
					ArrayFloat.D1 dataF = (ArrayFloat.D1) bufferArray;
			...
			case type_byte:
				...
				if (preconvert) {
					byte[] curr = (byte[]) bufferArray;
					curr[indices[0]] = toWriteByte;
					return curr;
					
				} else {
					ArrayByte.D1 dataByte = (ArrayByte.D1) bufferArray;
			...
			case type_char:
				...
				if (preconvert) {
					char[] curr = (char[]) bufferArray;
					curr[indices[0]] = toWriteC;
					return curr;
					
				} else {
					ArrayChar.D1 dataC = (ArrayChar.D1) bufferArray;
			...
			case type_short:
				...
				if (preconvert) {
					short[] curr = (short[]) bufferArray;
					curr[indices[0]] = toWriteS;
					return curr;
					
				} else {
					ArrayShort.D1 dataS = (ArrayShort.D1) bufferArray;
		...
		case 2: 
			...
			case type_int:
				...
				if (preconvert) {
					...
					ArrayInt.D2 dataI = (ArrayInt.D2) bufferArray;
			...
			case type_long:
				...
				if (preconvert) {
					...
					ArrayLong.D2 dataL = (ArrayLong.D2) bufferArray;
			...
			case type_bool:
				...
				if (preconvert) {
					...
					ArrayBoolean.D2 dataB = (ArrayBoolean.D2) bufferArray;
			...
			case type_double:
				...
				if (preconvert) {
					...
					ArrayDouble.D2 dataD = (ArrayDouble.D2) bufferArray;
			...
			case type_float:
				...
				if (preconvert) {
					...
					ArrayFloat.D2 dataF = (ArrayFloat.D2) bufferArray;
			...
			case type_byte:
				...
				if (preconvert) {
					...
					ArrayByte.D2 dataByte = (ArrayByte.D2) bufferArray;
			...
			case type_char:
				...
				if (preconvert) {
					...
					ArrayChar.D2 dataC = (ArrayChar.D2) bufferArray;
			...
			case type_short:
				...
				if (preconvert) {
					...
					ArrayShort.D2 dataS = (ArrayShort.D2) bufferArray;
		...
		case 3: 
			...
			case type_int:
				...
				if (preconvert) {
					...
					ArrayInt.D3 dataI = (ArrayInt.D3) bufferArray;
			...
			case type_long:
				...
				if (preconvert) {
					...
					ArrayLong.D3 dataL = (ArrayLong.D3) bufferArray;
			...
			case type_bool:
				...
				if (preconvert) {
					...
					ArrayBoolean.D3 dataB = (ArrayBoolean.D3) bufferArray;
			...
			case type_double:
				...
				if (preconvert) {
					...
					ArrayDouble.D3 dataD = (ArrayDouble.D3) bufferArray;
			...
			case type_float:
				...
				if (preconvert) {
					...
					ArrayFloat.D3 dataF = (ArrayFloat.D3) bufferArray;
			...
			case type_byte:
				...
				if (preconvert) {
					...
					ArrayByte.D3 dataByte = (ArrayByte.D3) bufferArray;
			...
			case type_char:
				...
				if (preconvert) {
					...
					ArrayChar.D3 dataC = (ArrayChar.D3) bufferArray;
			...
			case type_short:
				...
				if (preconvert) {
					...
					ArrayShort.D3 dataS = (ArrayShort.D3) bufferArray;
		...
		case 4: 
			...
			case type_int:
				...
				if (preconvert) {
					...
					ArrayInt.D4 dataI = (ArrayInt.D4) bufferArray;
			...
			case type_long:
				...
				if (preconvert) {
					...
					ArrayLong.D4 dataL = (ArrayLong.D4) bufferArray;
			...
			case type_bool:
				...
				if (preconvert) {
					...
					ArrayBoolean.D4 dataB = (ArrayBoolean.D4) bufferArray;
			...
			case type_double:
				...
				if (preconvert) {
					...
					ArrayDouble.D4 dataD = (ArrayDouble.D4) bufferArray;
			...
			case type_float:
				...
				if (preconvert) {
					...
					ArrayFloat.D4 dataF = (ArrayFloat.D4) bufferArray;
			...
			case type_byte:
				...
				if (preconvert) {
					...
					ArrayByte.D4 dataByte = (ArrayByte.D4) bufferArray;
			...
			case type_char:
				...
				if (preconvert) {
					...
					ArrayChar.D4 dataC = (ArrayChar.D4) bufferArray;
			...
			case type_short:
				...
				if (preconvert) {
					...
					ArrayShort.D4 dataS = (ArrayShort.D4) bufferArray;
...
//Creates a 2-Dimensional or 3-Dimensional NetCDF file
	...
	private boolean create( Object arg ) {
		Object[] args = (Object[]) arg;
