...
public class MASS 
{
	...
	/* finish( ) should not stop the entire main program.
        finally
        {
            System.exit(0);
        } 
	...
	*/
     ...
     */
    static void ea_setup(int handle, int ea_functionId,
                    Vector<int[]> ea_destinations) 
    {
            if (!INITIALIZED)
                return;
            MASS.ea_places = getPlaces( handle );
	...
	*/
    ...
    {      
		...
		//amala - commenting one line below
    ...
    private static void agentsManageAllPerThreadKillOrStartMigrate() 
    {
        ...
        { // null when not enough MASS.Place objects for thread
            ...
            while (placesIter.hasNext()) 
            {
				...
				if ( agent.alive == false ) { // added by M Fukuda on 11-22-13
				    // this agent must be terminated
				    // remove this agent from place.agents
				    vecAgent.remove(i--);

				    // remove this agent from MASS.Agents.bag( )
    ...
    private static class ParallelWriter extends Thread {
	String destinationHostName;
	Message msg;
	public ParallelWriter( String destinationHostName, Message msg ) {
    ...
    private static void receiveAndProcessRemoteAgentRequest( )
    {
        ...
        // send request to all nodes
	ParallelReader[] readers = new ParallelReader[requestsEntrySet.size( )];
	int nReaders = 0;
        while(it.hasNext())
        {
            req = (java.util.Map.Entry)it.next();
	...
	String destinationHostName;
	public ParallelReader( String destinationHostName ) {
	    this.destinationHostName = destinationHostName;
	}
	public void run( ) {
	    exchangeHelper[0].processAgentMigrateRequest(destinationHostName);
